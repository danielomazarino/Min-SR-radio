...
(() => {
  ...
  function renderPlayer() {
    ...
    const quality = qLabel
      ...
      : null;
    // Mode pill: LIVE at the live edge, otherwise the relative offset
    // ("−12 min"). Reads only the Phase 2A seekable state.
    const behind = live && cur.atLiveEdge === false && cur.distanceFromLiveEdge;
    const mode = live
      ? el('span', {
          class: `player-mode${behind ? ' behind' : ''}`,
          text: behind ? dvrOffsetLabel(cur.distanceFromLiveEdge) : 'LIVE',
          'aria-live': 'polite',
        })
      : null;
    ...
    if (!live) {
      ...
      // DVR seek row (Phase 3): only when the ACTIVE stream exposes a usable
      // seekable window. Maps the bar to the actual seekable range — never a
      // hard-coded duration. Playback state (paused/playing) is untouched.
      const bar = el('div', {
        class: 'seek-bar dvr-bar', role: 'slider',
        'aria-label': 'Spola i direktinspelningen',
        'aria-valuemin': 0, 'aria-valuemax': 100, 'aria-valuenow': 100,
        tabindex: '0',
      }, el('div', { class: 'seek-fill' }));
      const timeLeft = el('div', { class: 'player-time', text: '' });
      const liveBtn = el('button', {
        class: 'player-btn player-btn-live', type: 'button',
        'aria-label': 'Till Direkt', text: 'Till Direkt',
        onclick: seekToLive,
      });
      seekRow = el('div', { class: 'seek-row dvr-row' }, timeLeft, bar, liveBtn);
      const fill = bar.firstChild;

      const upd = () => {
        // Re-read state each tick — the window rolls and the live edge moves.
        const start = cur.seekableStart;
        const end = cur.seekableEnd;
        ...
        // Clamp the UI representation safely (no NaN/Infinity).
        const frac = Math.min(1, Math.max(0, (t - start) / (end - start)));
        fill.style.width = `${frac * 100}%`;
        bar.setAttribute('aria-valuenow', String(Math.round(frac * 100)));
        timeLeft.textContent = cur.atLiveEdge === false && cur.distanceFromLiveEdge
          ? dvrOffsetLabel(cur.distanceFromLiveEdge)
          : 'LIVE';
      };
      ...
      bar.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'ArrowLeft') {
          ...
          const frac = Math.min(1, Math.max(0,
            ((audioEl.currentTime || start) - start) / (end - start) - step));
          seekToWindowFraction(frac);
          upd();
