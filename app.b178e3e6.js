...
(() => {
  ...
  function playTrack(track) {
    ...
    // New playback session → the news auto-unfold may fire once again.
    newsAutoUnfolded = false;
  ...
  function stopAndClosePlayer() {
    ...
    $player.textContent = '';
    updatePlayingMarks();
    updateNewsFold(); // playback ended → fold Nyheter back (user request 2026-09-23)
  }
  ...
  function updatePlayingMarks() {
    ...
    // Nyheter unfold (user request 2026-09-23): when a program or podcast
    // starts playing, the news section unfolds; when playback stops, it
    // folds back. Manual toggle still works while playing.
    updateNewsFold();
  }

  // ---- Nyheter unfold/fold (user request 2026-09-23) ----
  // While something is playing, the news section unfolds automatically ONCE
  // per playback session; the section header is also a manual toggle
  // (chevron) so the user can fold/unfold at any time during playback. The
  // player stays on top (fixed, z-index above the page).
  // Module-level so the state survives renderHome() re-renders (favorites
  // save → boot → renderHome).
  let newsExpanded = false;
  let newsAutoUnfolded = false; // auto-unfold already fired for this session
  function updateNewsFold() {
    const playing = !audioEl.paused && state.current;
    if (playing && !newsExpanded && !newsAutoUnfolded) {
      // Auto-unfold ONCE when playback starts. After that the user's manual
      // choice wins — without this guard the auto-unfold would instantly
      // re-expand every time the user folded the section during playback
      // (verified live 2026-09-23: first manual fold was overridden).
      newsExpanded = true;
      newsAutoUnfolded = true;
    }
        ...
        if (!song || !song.title) {
          content.appendChild(el('div', { class: 'expand-row' },
            ...
            el('div', { class: 'expand-text' },
              ...
              el('div', { class: 'expand-title', text: cur._srProgramTitle || cur.title || '' }),
              cur._srProgramTitle && cur.title && cur._srProgramTitle !== cur.title
  ...
  function renderHome() {
    ...
    // Nyheter unfold (user request 2026-09-23): the header is a toggle
    // (chevron). The section auto-unfolds when a program/podcast plays
    // (updateNewsFold) and can be folded/unfolded manually at any time.
    // Collapsed = only the first news item peeks out as a hint.
    ...
    const newsToggle = el('button', {
      ...
      onclick: () => {
        // Manual toggle: the user's choice wins for this session — the
        // auto-unfold won't fire again until the next playback session.
        newsAutoUnfolded = true;
