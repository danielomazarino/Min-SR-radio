...
(() => {
   ...
   */
  function enableSwipeToClose(overlay, panel, close) {
    let startX = 0, startY = 0, dx = 0, dragging = false, horizontal = null, t0 = 0;
    ...
    panel.addEventListener('touchmove', (e) => {
      ...
      if (horizontal === null && (Math.abs(adx) > 8 || Math.abs(ady) > 8)) {
        horizontal = Math.abs(adx) > Math.abs(ady);
      }
      if (!horizontal) return;
      dx = adx;
      panel.style.transform = `translateX(${dx}px)`;
      // dim the backdrop proportionally
      overlay.style.background = `rgba(0,0,0,${Math.max(0, 0.0 - (dx / W()) * 0.4)})`;
    }, { passive: true });
    ...
    const finish = () => {
      ...
      const flick = elapsed < 250 && Math.abs(dx) > 40;
      if (Math.abs(dx) > W() * 0.35 || flick) {
        const dir = dx > 0 ? W() : -W();
        panel.style.transform = `translateX(${dir}px)`;
  ...
  function openSheet({ initialTab, onDone }) {
    ...
    // value is shown inside the slider thumb (CSS ::after reads data-value)
    const syncThumb = () => newsRange.setAttribute('data-value', newsLabel.textContent);
    syncThumb();
