...
(() => {
  ...
  function renderHome() {
    ...
    // ----- Channels & Podcasts: swipeable pages, 4 icons per row -----
    // Same look as before: one row of 4 visible. Swipe left/right (or the
    // dots) switches between the two pages.
    const pager = el('div', { class: 'fav-pager' });
    const pages = [
      { key: 'channels', title: 'Kanaler', favs: favs.channels, catalogue: state.channels },
      { key: 'podcasts', title: 'Poddar', favs: favs.podcasts, catalogue: state.podcasts },
    ];
    let pageIndex = 0;
    const pageEls = [];
    const dotsWrap = el('div', { class: 'pager-dots', role: 'tablist', 'aria-label': 'Kanaler och poddar' });
    const dots = pages.map((p, i) => {
      const dot = el('button', {
        class: 'pager-dot', type: 'button', role: 'tab',
        'aria-label': p.title, 'aria-selected': String(i === 0),
        onclick: () => goToPage(i),
      });
    ...
    function buildPage(page) {
      const sec = el('section', { class: 'section fav-page', 'aria-label': page.title },
        el('h2', { class: 'section-title', text: page.title }));
      const row = el('div', { class: 'icon-row' });
      for (let i = 0; i < MAX_FAVORITES; i++) {
        const item = page.catalogue.find((c) => c.id === page.favs[i]);
        if (!item) {
          row.appendChild(el('div', { class: 'icon-empty', 'aria-hidden': 'true' }));
          continue;
        }
        const isPod = page.key === 'podcasts';
        const btn = el('button', {
    ...
    function goToPage(i) {
      ...
      pageEls.forEach((el2, j) => {
        el2.style.transform = `translateX(${(j - pageIndex) * 100}%)`;
        el2.classList.toggle('active', j === pageIndex);
      });
      dots.forEach((d, j) => d.setAttribute('aria-selected', String(j === pageIndex)));
    }

    pages.forEach((p, i) => {
      const pageEl = buildPage(p);
      pageEls.push(pageEl);
    ...
    // Horizontal swipe on the pager switches page
    let sx = 0, sy = 0, sdx = 0, swiping = null;
    pager.addEventListener('touchstart', (e) => {
      if (e.touches.length !== 1) return;
      sx = e.touches[0].clientX; sy = e.touches[0].clientY; sdx = 0; swiping = null;
      pageEls.forEach((el2) => { el2.style.transition = 'none'; });
    }, { passive: true });
    pager.addEventListener('touchmove', (e) => {
      if (swiping === false) return;
      const adx = e.touches[0].clientX - sx;
      const ady = e.touches[0].clientY - sy;
      if (swiping === null && (Math.abs(adx) > 10 || Math.abs(ady) > 10)) {
        swiping = Math.abs(adx) > Math.abs(ady);
        if (!swiping) return;
      }
      if (!swiping) return;
      sdx = adx;
      const atEdge = (pageIndex === 0 && sdx > 0) || (pageIndex === pages.length - 1 && sdx < 0);
      const damp = atEdge ? 0.3 : 1;
      pageEls.forEach((el2, j) => {
        el2.style.transform = `translateX(calc(${(j - pageIndex) * 100}% + ${sdx * damp}px))`;
      });
    }, { passive: true });
    pager.addEventListener('touchend', () => {
      pageEls.forEach((el2) => { el2.style.transition = ''; });
      if (swiping !== true) return;
      if (sdx < -60 && pageIndex < pages.length - 1) goToPage(pageIndex + 1);
      else if (sdx > 60 && pageIndex > 0) goToPage(pageIndex - 1);
      else goToPage(pageIndex);
    }, { passive: true });

    // ----- Selected favorites section: shows what's chosen, with custom sorting -----
    // Order here = order on the home screen (first = leftmost).
    const selectedSection = el('section', { class: 'section' },
      el('h2', { class: 'section-title', text: 'Valda favoriter' }));
    ...
    function buildSelectedGroup(kind, title, catalogue) {
      const group = el('div', { class: 'selected-group' },
        el('h3', { class: 'selected-group-title', text: title }));
      const list = favs[kind];
      if (!list.length) {
        group.appendChild(el('div', { class: 'selected-empty', text: 'Inga valda ännu — välj under Inställningar.' }));
        return group;
      }
      list.forEach((id, pos) => {
        const item = catalogue.find((c) => c.id === id);
        if (!item) return;
        const rowEl = el('div', { class: 'selected-item' },
        ...
        controls.appendChild(el('button', {
          ...
          onclick: () => { if (moveFavorite(favs, kind, id, 'up')) renderHome(); },
        }));
        controls.appendChild(el('button', {
          class: 'selected-btn', type: 'button',
          ...
          onclick: () => { if (moveFavorite(favs, kind, id, 'down')) renderHome(); },
        }));
