...
(() => {
  ...
  const CAPS = {
    ...
    canPlayFlac: true,
    // HLS is Phase 2 — no HLS candidates are emitted yet, but the capability
    // flag is defined now so Phase 2 only extends the table/resolver.
    nativeHls: false, // set in Phase 2 (Safari canPlayType check)
    hlsjs: false,     // set in Phase 2 (MSE + non-Firefox)
    canPlayHls: false,
  };

  // Static stream table (Phase 1). Only DIRECT streams for now — HLS entries
  // are added in Phase 2. FLAC entries stay hand-maintained (undocumented
  // by SR); liveaudio.url (MP3) is appended by the resolver for every channel.
  const STREAM_TABLE = {
    2562: [
  ...
  // Order is IDENTICAL to the previous implementation:
  //   P2 Musik: FLAC → (iOS: AAC-320) → MP3
  //   others:   (iOS: AAC-320) → MP3
  function resolveStreams(channel) {
    if (!channel || !channel.liveaudioUrl) return [];
    const cands = [];

    // Static table entries (FLAC etc.), filtered by platform capability.
