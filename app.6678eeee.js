...
(() => {
  ...
  //      plays in Chromium & Safari. Undocumented by SR → MP3 must stay as
  //      fallback. NOTE: the edge slug family 'p2' belongs to P2 Musik
  //      (SR's own 2562.hls lists p2/* variants; 163.hls lists p2sm/*),
  //      so p2-flac is P2 Musik's stream — NOT P2 (163)'s.
  ...
  const P2_MUSIK_FLAC_URL = 'https://edge1.sr.se/p2-flac';

  function liveCandidates(channel) {
    const cands = [];
    if (channel.id === 2562 && channel.liveaudioUrl) cands.push(P2_MUSIK_FLAC_URL);
  ...
  // format badge pulses with a "buffrar" suffix — zero extra layout space.
  // Reverts to the plain format label once audio flows again.
  function setBadgeBuffering(buffering) {
    ...
    const fmt = badge.dataset.format || streamFormatLabel(state.current?.audioUrl) || '';
