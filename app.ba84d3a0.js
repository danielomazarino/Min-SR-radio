...
(() => {
  ...
  function renderHome() {
    ...
    // ----- Channels & Podcasts: same look as before — two sections, 4 icons
    // visible per row. Swipe left/right within a section reveals additional
    // selections beyond the first four (pages of 4).
    function buildIconSection(kind, title, catalogue) {
      const isPod = kind === 'podcasts';
      const list = favs[kind];
      const pages = [];
      for (let i = 0; i < list.length; i += MAX_FAVORITES) {
        pages.push(list.slice(i, i + MAX_FAVORITES));
      }
      if (!pages.length) pages.push([]); // one empty page so the row renders

      const sec = el('section', { class: 'section', 'aria-label': title },
        el('h2', { class: 'section-title', text: title }));
      const viewport = el('div', { class: 'icon-viewport' });
      const track = el('div', { class: 'icon-track' });
      const pageEls = [];
      let pageIndex = 0;

      function buildPage(pageFavIds) {
        const row = el('div', { class: 'icon-row' });
        for (let i = 0; i < MAX_FAVORITES; i++) {
          const item = catalogue.find((c) => c.id === pageFavIds[i]);
          if (!item) {
        ...
        return el('div', { class: 'icon-page' }, row);
      }
      ...
      // Horizontal swipe within the section reveals more selections
  ...
  function openSheet({ initialTab, onDone }) {
    ...
    // home screen (first = leftmost). Lives only in this settings view.
