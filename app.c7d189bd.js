...
(() => {
  ...
  function renderPlayer() {
    ...
    // the CLOCK TIME of the position being heard (Swedish timezone) — the
    // relative offset lives on the seek row, so nothing is duplicated.
    ...
    if (behind) {
      ...
      modeText = d ? dvrClockLabel(d) : dvrOffsetLabel(cur.distanceFromLiveEdge);
    }
    const mode = live
      ? el('span', {
    ...
    if (!live) {
      ...
      const endDrag = (e) => {
        ...
        seekToWindowFraction(fracFromEvent(e));
        upd();
      };
