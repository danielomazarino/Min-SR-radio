...
(() => {
  ...
  function openAbout() {
    ...
    const items = [
      ...
      ['Ändra favoriter', 'Välj exakt 4 kanaler och 4 poddar — byt enskilda val när som helst, du behöver aldrig börja om.'],
    ];
  ...
  function openSheet({ initialTab, onDone }) {
    ...
    function updateCounter() {
      ...
      counter.textContent = tab === 'channels'
        ? `Välj ${MAX_FAVORITES} kanaler (${n}/${MAX_FAVORITES} valda)`
        : `Välj ${MAX_FAVORITES} poddar (${n}/${MAX_FAVORITES} valda)`;
    }

    function doneBtnState() {
      doneBtn.disabled = picks.channels.length !== MAX_FAVORITES || picks.podcasts.length !== MAX_FAVORITES;
    }
    ...
    function setTitle() {
      title.textContent = tab === 'channels' ? 'Välj kanaler' : 'Välj poddar';
    }
    ...
    doneBtn = el('button', { class: 'sheet-done', type: 'button', text: 'Klar',
      onclick: () => { closeSheet(); onDone?.(); } });

    // News count setting (4–20, step 2)
    ...
    const newsSetting = el('div', { class: 'setting-row' },
      el('div', { class: 'setting-text' },
        el('div', { class: 'pick-name', text: 'Antal nyheter' }),
        el('div', { class: 'pick-sub', text: 'Visa fler nyheter i listan (4–20)' })),
      el('div', { class: 'setting-control' }, newsLabel, newsRange));

    // About/help button ABOVE the channel & podcast selection
  ...
  async function boot() {
    ...
    if (favs.channels.length < MAX_FAVORITES || favs.podcasts.length < MAX_FAVORITES) {
      $main.textContent = '';
      $main.appendChild(el('div', { class: 'state-msg' },
        el('div', { text: 'Välkommen! Välj dina 4 favoritkanaler och 4 favoritpoddar för att komma igång.' }),
        el('button', {
