...
(() => {
  ...
  // While something is playing, the news section unfolds automatically so
  // the user sees the news below the player. The section header is also a
  // manual toggle (chevron) so the user can fold/unfold at any time during
  // playback. The player stays on top (fixed, z-index above the page).
  // Module-level so the state survives renderHome() re-renders (favorites
  // save → boot → renderHome).
  let newsExpanded = false;
  let newsAutoFolded = false; // auto-unfold happened → auto-fold on stop
  function updateNewsFold() {
    const playing = !audioEl.paused && state.current;
    if (playing && !newsExpanded) {
      newsExpanded = true;
      newsAutoFolded = true; // we unfolded it — we fold it back on stop
    } else if (!playing && newsExpanded && newsAutoFolded) {
      newsExpanded = false;
      newsAutoFolded = false;
    }
  ...
  function renderHome() {
    ...
    const newsToggle = el('button', {
      ...
      onclick: () => {
        // Manual toggle: clear the auto flag so stopping playback doesn't
        // fold a section the user unfolded by hand.
        newsAutoFolded = false;
